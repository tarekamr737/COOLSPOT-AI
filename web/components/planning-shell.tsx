...
function EvidencePanel({ candidate, site, methodology, explanationMode, siteLoading, explanation, explanationLoading, explanationError, onExplain }: EvidencePanelProps) {
  ...
  return <aside aria-busy={siteLoading} className={styles.evidencePanel} aria-labelledby="evidence-title">
    ...
    <dl className={styles.evidenceList}><div><dt>Observed heat</dt><dd>{tile.heat.average_temperature_c.toFixed(2)} °C tile average · {formatDate(heatProvenance.active_analysis_date)}</dd></div><div><dt>Heat persistence</dt><dd>{tile.heat.persistence_hours.toFixed(2)} hours · {formatDate(heatProvenance.active_analysis_date)}</dd></div><div><dt>Historical exceedance</dt><dd>{tile.heat.exceedance_hours.toFixed(2)} hours {heatProvenance.exceedance_direction} {heatProvenance.exceedance_threshold_c} °C · {formatDate(heatProvenance.exceedance_analysis_date)}</dd></div><div><dt>Published patronage activity</dt><dd>{tile.exposure.published_patronage_activity?.toFixed(2) ?? "Not available"}</dd></div><div><dt>Vulnerability context</dt><dd>{tile.scores.vulnerability.toFixed(3)} modeled score</dd></div></dl>
