...
function RecommendationRail({ candidates, portfolio, activeCandidateId, onSelect }: RecommendationRailProps) {
  ...
  const scoreByCandidate = new Map(
    portfolio.selected_candidate_scores.map((item) => [item.candidate_id, item]),
  );
  return <aside className={styles.recommendationRail} aria-labelledby="recommendations-title">
    ...
    <ol className={styles.recommendationList}>{candidates.map((candidate, index) => <li className={styles.recommendation} key={candidate.id}>
      <button aria-current={candidate.id === activeCandidateId ? "true" : undefined} className={styles.recommendationButton} onClick={() => onSelect(candidate)} type="button">
        <span className={styles.rank}>{String(index + 1).padStart(2, "0")}</span><span className={styles.recommendationBody}><span className={styles.interventionLabel}>{interventionLabel(candidate.intervention_type)}</span><strong>{candidate.site_name}</strong><span className={styles.recommendationMeta}><span>{compactCurrency(candidate.planning_cost_usd)} LA allowance</span><span>{(scoreByCandidate.get(candidate.id)?.modeled_impact_score ?? impact(candidate)).toFixed(3)} impact</span>{robustnessBySite.get(candidate.site_id) ? <span className={styles.robustnessLabel}>Selected in {robustnessBySite.get(candidate.site_id)?.presets_selected}/{robustnessBySite.get(candidate.site_id)?.presets_tested} planning scenarios</span> : null}</span></span>
      </button>
    </li>)}</ol>
  </aside>;
}
...
function BudgetBar({ budget, portfolio, methodology, optimizing, onCommit, onPreview, onScenarioChange }: BudgetBarProps) {
  ...
  return <section className={styles.budgetBar} aria-labelledby="budget-title">
    ...
    <div className={styles.budgetControl}><div className={styles.budgetScale} aria-label="Budget presets">{optimization.budget_presets_usd.map((preset) => <button aria-pressed={budget === preset} className={budget === preset ? styles.activeBudget : undefined} disabled={optimizing} key={preset} onClick={() => onCommit(preset)} type="button">{compactCurrency(preset)}</button>)}</div>
      <label className={styles.sliderLabel}><span className="sr-only">Custom budget</span><input aria-valuetext={currency.format(budget)} disabled={optimizing} id="custom-budget" max={optimization.custom_budget_max_usd} min={optimization.custom_budget_min_usd} name="budget_usd" onChange={(event) => onPreview(Number(event.currentTarget.value))} onKeyUp={(event) => onCommit(Number(event.currentTarget.value))} onPointerUp={(event) => onCommit(Number(event.currentTarget.value))} step={50_000} type="range" value={budget} /></label>
    </div>
    <label className={styles.scenarioControl}><span>Planning priority</span><select aria-label="Planning priority" disabled={optimizing} id="planning-priority" name="scoring_preset" onChange={(event) => onScenarioChange(event.currentTarget.value as ScoringPreset)} value={portfolio.scoring_preset}>{scoringPresets.map((preset) => <option key={preset} value={preset}>{scenarioLabels[preset]}</option>)}</select><small>H {Math.round(weights.heat * 100)} · E {Math.round(weights.exposure * 100)} · V {Math.round(weights.vulnerability * 100)} · O {Math.round(weights.cooling_opportunity * 100)}</small></label>
...
function MapWorkspace({ budget, data, layer, activeLayer, layerLoading, optimizing, activeCandidateId, streetContext, streetLoading, streetVisible, onCloseStreet, onBudgetCommit, onBudgetPreview, onLayerChange, onScenarioChange, onSelectSite }: MapWorkspaceProps) {
  ...
  return <section className={styles.mapWorkspace} aria-labelledby="map-title" id="map-workspace" tabIndex={-1}>
...
type EvidencePanelProps = { candidate: Candidate; site: Site; portfolio: Portfolio; methodology: Methodology; explanationMode: DataStatus["explanation_mode"]; siteLoading: boolean; explanation?: Explanation; explanationLoading: boolean; explanationError: string | null; onExplain: () => void; onViewStreet: () => void };
...
function StreetEvidenceSummary({ evidence, interventionType, onViewImages }: { evidence: Site["street_view_evidence"]; interventionType: Candidate["intervention_type"]; onViewImages: () => void }) {
  if (!evidence) {
  ...
  return <section className={styles.streetEvidenceSection} aria-labelledby="street-evidence-title">
    <div className={styles.streetEvidenceHeading}><div><p className={styles.eyebrow}>Street screening evidence</p><h3 id="street-evidence-title">Observed inputs and derivation</h3></div><button onClick={onViewImages} type="button">View street images</button></div>
...
function ThermalStressSummary({ context }: { context: Candidate["thermal_stress_context"] }) {
  if (!context) return null;
  return <section className={styles.streetEvidenceSection} aria-labelledby="thermal-context-title">
    <div className={styles.streetEvidenceHeading}><div><p className={styles.eyebrow}>Point thermal context</p><h3 id="thermal-context-title">Weather conditions at this finalist</h3></div><span>Source complete · {context.evidence_confidence.configured_fields_present}</span></div>
    <p className={styles.streetEvidenceProvenance}>{formatDate(context.observed_at.slice(0, 10))} · {context.vendor_timezone} · {context.latitude.toFixed(5)}, {context.longitude.toFixed(5)} · <a href="https://docs-api.fortyguard.com/docs/environmental-parameters" rel="noreferrer" target="_blank">FortyGuard source</a></p>
    <dl className={styles.streetMetricGrid}><div><dt>Apparent temperature</dt><dd>{context.apparent_temperature_c.toFixed(2)} °C</dd></div><div><dt>Relative humidity</dt><dd>{context.relative_humidity_percent.toFixed(1)}%</dd></div><div><dt>Clear-sky GHI</dt><dd>{context.clear_sky_ghi_vendor_value.toFixed(2)} vendor value</dd></div></dl>
    <p className={styles.streetEvidenceLimit}>{context.evidence_confidence.limitation}</p>
  </section>;
}

function EvidencePanel({ candidate, site, portfolio, methodology, explanationMode, siteLoading, explanation, explanationLoading, explanationError, onExplain, onViewStreet }: EvidencePanelProps) {
  ...
  const heatProvenance = methodology.heat_provenance;
  const peakProvenance = methodology.peak_hour_provenance;
  const heatWeights = methodology.scoring.heat_weights;
  const selectedScore = portfolio.selected_candidate_scores.find((item) => item.candidate_id === candidate.id);
  const robustness = portfolio.site_robustness.find((item) => item.site_id === candidate.site_id);
  return <aside aria-busy={siteLoading} className={styles.evidencePanel} aria-labelledby="evidence-title">
    ...
    <div className={styles.interventionCallout}><p><span className={`${styles.evidenceTag} ${styles.assumptionTag}`}>Planning assumption</span><span>Pacoima / Los Angeles price reference</span></p><h3>{intervention.label}</h3><div><span>{currency.format(intervention.planning_cost.estimate_usd)} planning allowance</span><span>{compactCurrency(intervention.planning_cost.low_usd)}–{compactCurrency(intervention.planning_cost.high_usd)} range</span></div><p className={styles.priceBasis}>{intervention.planning_cost.unit}. {intervention.planning_cost.basis}</p>{sources.filter((source) => intervention.planning_cost.source_ids.includes(source.id)).map((source) => <a href={source.url} key={source.id} rel="noreferrer" target="_blank">Price basis: {source.publisher}</a>)}</div>
    <p className={styles.evidenceCategory}><span className={`${styles.evidenceTag} ${styles.modeledTag}`}>Modeled</span><span>Derived screening scores</span></p>
    <section className={styles.impactSection}><div><p className={styles.eyebrow}>Modeled impact score</p><strong>{(selectedScore?.modeled_impact_score ?? impact(candidate)).toFixed(3)}</strong></div><p>Relative planning score under the {scenarioLabels[portfolio.scoring_preset]} weights. It is not a temperature forecast or guaranteed outcome.</p></section>
    <dl className={styles.decisionFactors} aria-label="Decision screening factors"><div><dt>Scenario priority</dt><dd>{selectedScore?.scenario_priority_score.toFixed(3) ?? candidate.benefit_score.toFixed(3)}</dd></div><div><dt>Intervention evidence</dt><dd>{candidate.suitability_score.toFixed(3)}</dd></div><div><dt>Feasibility <small className={`${styles.evidenceTag} ${styles.fieldTag}`}>Field verification required</small></dt><dd>{candidate.feasibility_score.toFixed(3)}</dd></div><div><dt>Evidence confidence</dt><dd>{candidate.confidence.toFixed(3)}</dd></div><div><dt>Scenario robustness</dt><dd>{robustness ? `${robustness.presets_selected}/${robustness.presets_tested}` : "Not computed"}</dd></div></dl>
    <p className={styles.evidenceCategory}><span className={`${styles.evidenceTag} ${styles.verifiedTag}`}>Verified evidence</span><span>Observed and published evidence</span></p>
    <dl className={styles.evidenceList}><div><dt>Observed heat</dt><dd>{tile.heat.average_temperature_c.toFixed(2)} °C tile average · {formatDate(heatProvenance.active_analysis_date)}</dd></div><div><dt>Heat persistence</dt><dd>{tile.heat.persistence_hours.toFixed(2)} hours · {formatDate(heatProvenance.active_analysis_date)}</dd></div><div><dt>Historical exceedance</dt><dd>{tile.heat.exceedance_hours.toFixed(2)} hours {heatProvenance.exceedance_direction} {heatProvenance.exceedance_threshold_c} °C · {formatDate(heatProvenance.exceedance_analysis_date)}</dd></div><div><dt>Peak heat observed around</dt><dd>{String(tile.heat.peak_heat_hour_utc).padStart(2, "0")}:00 {peakProvenance.timezone} · {formatDate(peakProvenance.analysis_date)}</dd></div><div><dt>Published patronage activity</dt><dd>{tile.exposure.published_patronage_activity?.toFixed(2) ?? "Not available"}</dd></div><div><dt>Vulnerability context</dt><dd>{tile.scores.vulnerability.toFixed(3)} modeled score</dd></div></dl>
    <ThermalStressSummary context={candidate.thermal_stress_context} />
    <StreetEvidenceSummary evidence={site.street_view_evidence} interventionType={candidate.intervention_type} onViewImages={onViewStreet} />
    <details className={styles.scoreBreakdown}><summary>How the priority score is calculated</summary><h4>Heat severity inputs</h4><dl><div><dt>Temperature</dt><dd>{tile.heat.temperature_score.toFixed(3)} · {(heatWeights.temperature * 100).toFixed(0)}% weight</dd></div><div><dt>Persistence</dt><dd>{tile.heat.persistence_score.toFixed(3)} · {(heatWeights.persistence * 100).toFixed(0)}% weight</dd></div><div><dt>Historical exceedance</dt><dd>{tile.heat.exceedance_score.toFixed(3)} · {(heatWeights.exceedance * 100).toFixed(0)}% weight</dd></div></dl><h4>Priority inputs</h4><dl>{Object.entries(tile.scores).map(([name, value]) => <div key={name}><dt>{name.replaceAll("_", " ")}</dt><dd>{value.toFixed(3)}{methodology.scoring.priority_weights[name] !== undefined ? ` · ${(methodology.scoring.priority_weights[name] * 100).toFixed(0)}% weight` : ""}</dd></div>)}</dl><p>Observed and published inputs are normalized to 0–1, then combined with the published weights. Historical exceedance uses a different source date, disclosed below. The intervention impact also applies feasibility and confidence screening.</p></details>
    <section className={styles.confidenceSection}><div className={styles.confidenceHeading}><span>Evidence confidence</span><strong>{site.street_view_evidence ? "Exact-site street screen" : "Unverified screening"} · {candidate.confidence.toFixed(3)}</strong></div><div aria-label="Evidence confidence" aria-valuemax={1} aria-valuemin={0} aria-valuenow={candidate.confidence} className={styles.confidenceTrack} role="meter"><span style={{ width: `${candidate.confidence * 100}%` }} /></div><p>{intervention.uncertainty.summary}</p></section>
...
export function PlanningShell() {
  ...
  return <div className={styles.appShell}><a className={styles.skipLink} href="#map-workspace">Skip to map workspace</a>{tourVisible ? <ProductTour onClose={closeTour} /> : null}<TopBar onRefreshComplete={retry} onTour={() => setTourVisible(true)} pilot={data.pilot} status={data.status} />{operationError ? <div className={styles.operationError} role="alert"><span>{operationError}</span><button aria-label="Dismiss error" onClick={() => setOperationError(null)} type="button">Dismiss</button></div> : null}<main className={styles.workspace}>
