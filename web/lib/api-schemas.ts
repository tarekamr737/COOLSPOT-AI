...
export const candidateSchema = z.object({
  ...
  value_explanation: z.object({
    formula: z.literal("priority_score × suitability_score × feasibility_score × confidence_score"),
    factors: z.object({
    ...
    modeled_benefit_score: z.number().min(0).max(1),
    suitability_basis: z.array(z.string().min(1)).min(1),
    limitation: z.string().min(1),
  }),
